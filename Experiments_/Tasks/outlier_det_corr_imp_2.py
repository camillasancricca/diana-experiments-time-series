...
def save_results(od_method, oc_method, imp_method, percentage, r2):
    path = "../Results/Air_Quality/knn_regression_mix_outliers_none_2.csv"
...
outlier_correction_methods = ['none']
    ...
    for oc_method in outlier_correction_methods:
        for imp_method in imputation_methods:
            ...
            for message in consumer:
                ...
                if oc_method == 'drop':
                    if od_method in ['z','hst'] and (outlier is True or o is True):
                        df = df.drop(pippo)
                        pippo.pop()
                    if od_method in ['lof','iforest']:
                        df = df.drop(pippo)
