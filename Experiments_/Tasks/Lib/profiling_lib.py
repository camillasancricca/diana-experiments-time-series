...
def check_variation(df, mean, std_dev, count, types, columns):
    s = None
    f = open("../Results/Anomaly_values.txt", 'a')
...
def create_files(columns, name=""):
    ...
    path = "../Results/mean" + name + ".csv"
    f = open(path, 'w')
    w = writer(f)
    w.writerow(c)
    f.close()

    path = "../Results/std_dev" + name + ".csv"
    f = open(path, 'w')
    w = writer(f)
    w.writerow(c)
    f.close()

    f = open('../Results/Anomaly_values.txt', 'w')
    f.close()

    f = open('../Results/FD_eff_a_priori.txt','w')
    f.close()
