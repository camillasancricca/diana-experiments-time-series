...
def save_results(method, percentage, precision, recall, f1):
    path = "../Results/Chlorine_Concentration/outlier_detection_2.csv"
...
with open('../Datasets/outliers_index_2.pkl', 'rb') as pick:
...
for method in methods:
    ...
    for message in consumer:
        ...
        if method == 'lof':
            ...
            df = pd.concat([df, pd.DataFrame(row,columns=columns[1:-1])], ignore_index=True)
            if len(df) > window:
